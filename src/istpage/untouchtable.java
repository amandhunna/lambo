...
public class untouchtable   {
Button ba= new Button("Back");
    Button pr= new Button("Print");

    
 untouchtable(Stage stage){   
       // Stage stage = new Stage();
        System.out.println("hello untouchtable class cons");
        Scene scene = new Scene(new Pane());
        stage.setTitle("untouchtable Case Details");
        stage.setWidth(5500);
        stage.setHeight(2100);
       final Label label = new Label("Untouched Case Details");
...
Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/db1","root","C@t143:)");  //'untouched'
Statement stmt=con.createStatement();  
ResultSet rs=stmt.executeQuery("SELECT * FROM db1.details LEFT JOIN db1.tech_window ON db1.details.serialdate = db1.tech_window.serialdate where tech_window.macstatus='untouched' order by fdate desc;");
